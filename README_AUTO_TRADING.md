...
4. **텔레그램 봇 연동 (버튼 클릭형 인터페이스)**
   - 매수/매도 체결 알림 (ON/OFF 가능)
   - 실시간 잔고 및 손익 현황 조회 (버튼 클릭)
   - 실시간 미실현손익 조회 (버튼 클릭)
   - 시스템 시작/종료 제어 (버튼 클릭)
   - 시스템 상태 확인 (버튼 클릭)
   - 양방향 통신으로 편리한 조작
...
**중요**: python-telegram-bot 버전 20.0 이상 필요
...
시스템 시작 후 텔레그램에서 다음과 같이 사용:

1. **봇과 대화 시작**: 생성한 봇을 검색하여 대화 시작
2. **/start** 명령어 입력 → 메인 메뉴 표시
3. **버튼 클릭**으로 다음 기능 사용:
   - 🤖 **봇 시작/종료**: 자동매매 시스템 제어
   - 💰 **실시간 잔고**: 현재 보유 종목 및 잔고 조회
   - 📊 **미실현손익**: 실시간 평가손익 상세 조회
   - 🔔 **체결알림**: 체결 알림 ON/OFF 토글
   - 📈 **시스템 상태**: 현재 시스템 운영 상태 확인
   - 🔄 **새로고침**: 메인 메뉴로 돌아가기
   - ❓ **도움말**: 사용 가이드 표시
...
체결알림이 ON 상태일 때 다음 알림이 자동으로 전송됩니다:
- 🔵 매수 체결 알림
- 🟢/🔴 매도 체결 알림 (수익률 포함)
