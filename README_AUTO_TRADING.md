...
3. **텔레그램 봇 (Enhanced)**: 버튼 클릭형 인터페이스로 실시간 제어 및 조회
   - 💼 실시간 잔고 조회
   - 📊 미실현 손익 확인
   - 🔔 체결 알림 ON/OFF 설정
   - 📢 주문 알림 ON/OFF 설정
...
```bash
pip install pandas requests websockets pyyaml pycryptodome python-telegram-bot
```

또는 uv 사용 시:

```bash
uv pip install pandas requests websockets pyyaml pycryptodome python-telegram-bot
```

**중요**: `python-telegram-bot` 라이브러리는 버튼 클릭형 텔레그램 봇 기능을 위해 필수입니다.
...
**새로운 기능**: 버튼 클릭형 인터페이스로 실시간 제어 가능
- `/start` - 봇 시작 및 환영 메시지
- `/menu` - 메인 메뉴 표시 (버튼 인터페이스)
- `/help` - 도움말 표시
...
- **📊 미실현손익**: 실시간 평가손익 확인
- **🔔 체결알림 설정**: 체결 알림 ON/OFF 토글
- **📢 주문알림 설정**: 주문 알림 ON/OFF 토글
- **▶️ 시스템 시작**: 자동매매 시작
- **⏹ 시스템 종료**: 자동매매 중지
- **🔄 새로고침**: 메뉴 새로고침
- **❓ 도움말**: 도움말 표시
...
- **주문 알림**: 주문 접수 시 (ON/OFF 설정 가능)
- **체결 알림**: 체결 완료 시 (ON/OFF 설정 가능)
- **잔고 알림**: 잔고 업데이트 시 (30초마다)

### 3. 실시간 조회 기능 (버튼 클릭)

#### 💼 실시간 잔고 조회
버튼을 클릭하면 즉시 다음 정보를 확인할 수 있습니다:
- 전체 요약 (총 매입금액, 총 평가금액, 평가손익, 수익률)
- 보유 종목 목록 (최대 10개)
- 각 종목의 수량, 매입가, 현재가, 평가손익
...
- **v2.0.0** (2026-01-12)
  - 🎨 텔레그램 봇 버튼 클릭형 인터페이스 구현
  - 💼 실시간 잔고 조회 버튼 기능
  - 📊 미실현 손익 조회 버튼 기능
  - 🔔 체결 알림 ON/OFF 설정 기능
  - 📢 주문 알림 ON/OFF 설정 기능
  - ▶️⏹ 시스템 시작/종료 버튼 제어
  - 🤖 python-telegram-bot 라이브러리 통합
